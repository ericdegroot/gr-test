...
const std::string default_access_code =
  "1010110011011101101001001110001011110010100011000010000011111100";
const std::string packed_default_access_code = "\xAC\xDD\xA4\xE2\xF2\x8C\x20\xFC";
const size_t packed_default_access_code_length = 8;

const std::string preamble = "1010010011110010";
const std::string packed_preamble = "\xA4\xF2";
const size_t packed_preamble_length = 2;

#define VERBOSE 0

void conv_1_0_string_to_packed_binary_string(std::string& ps, bool& padded,
					     const char* s, size_t n)
{
  size_t nn = n, r = nn % 8, npad = 0;
  if (r != 0) {
    npad = 8 - r;
    nn += npad;
    padded = true;
...
void conv_packed_binary_string_to_1_0_string(std::string& bs, const char* s,
					     size_t n)
{
  bs.reserve(bs.length() + n * 8);
...
{
  uint16_t n_payload_len = htons(payload_len);
  size_t s = sizeof(n_payload_len);

  header.reserve(header.length() + s);

  for (int i = 0; i < 2; i++) {
    for (size_t j = 0; j < s; j++) {
      header.push_back((unsigned char) ((n_payload_len >> (8 * j)) & 0xFF));
    }
  }
}
...
int _npadding_bytes(int pkt_byte_len, int samples_per_symbol, int bits_per_symbol) {
  int modulus = 128;
  int byte_modulus = boost::math::lcm(modulus/8, samples_per_symbol) *
...
{
  uint32_t crc = digital_crc32((const unsigned char*) payload, payload_len);
  uint32_t n_crc = htonl(crc);

  //if (VERBOSE)
  //  fprintf(stderr, "make_packet: payload_len=%d, crc=%.8X, n_crc=%.8X\n", payload_len, crc, n_crc);

  std::string packed_access_code;
  bool padded;
  conv_1_0_string_to_packed_binary_string(packed_access_code, padded,
					  access_code.data(),
					  access_code.length());

  std::string header;
  make_header(header, payload_len + 4);

  packet.reserve(packet.length() + packed_preamble_length +
		 packed_access_code.length() + header.length() + payload_len +
		 4 + 1);

  packet.append(packed_preamble);
  packet.append(packed_access_code);
  packet.append(header);

  std::string payload_with_crc(payload, payload_len);
  payload_with_crc.reserve(payload_len + 4);

  // append network order crc
  payload_with_crc.push_back((unsigned char) ((n_crc >> 0) & 0xFF));
  payload_with_crc.push_back((unsigned char) ((n_crc >> 8) & 0xFF));
  payload_with_crc.push_back((unsigned char) ((n_crc >> 16) & 0xFF));
  payload_with_crc.push_back((unsigned char) ((n_crc >> 24) & 0xFF));

  std::string whitened_payload_with_crc;
  if (whitening) {
    whitened_payload_with_crc = whiten(payload_with_crc.data(),
				       payload_with_crc.length(), whitener_offset, showWhiten);
    showWhiten = false;

    if (toast5) {
      fprintf(stderr, "rec-whiten: ");
      for (int i = 0; i < 30; i++) {
	fprintf(stderr, "%02X", (unsigned char) whitened_payload_with_crc[i]);
      }
      fprintf(stderr, "\n");

      std::string dewhitened = whiten(whitened_payload_with_crc.data(),
				      whitened_payload_with_crc.length(),
				      whitener_offset, showWhiten);
      fprintf(stderr, "dee-whiten: ");
      for (int i = 0; i < 30; i++) {
	fprintf(stderr, "%02X", (unsigned char) dewhitened[i]);
      }
      fprintf(stderr, "\n");
      toast5 = false;
    }
  } else {
    whitened_payload_with_crc = payload_with_crc;
  }
					 
  packet.append(whitened_payload_with_crc);

  packet.push_back('\x55');

  if (VERBOSE || toast4) {
    fprintf(stderr, "make_packet: crc=%u, crc=%.8X, len=%d\n", crc, crc, payload_len);
  ...
  if (pad_for_usrp) {
    int npadding = _npadding_bytes(packet.length(), samples_per_symbol,
				   bits_per_symbol);
...
bool showWhitenUnmake = true;
bool toast3 = true;

void unmake_packet(std::string& payload, bool& ok,
		   const char* whitened_payload_with_crc,
		   size_t payload_with_crc_len, int whitener_offset,
		   bool dewhitening)
{
  size_t payload_len = payload_with_crc_len - 4;
  if (payload_len < 0) {
    ok = false;
    return;
  }

  std::string payload_with_crc;
  if (dewhitening) {
    payload_with_crc = whiten(whitened_payload_with_crc,
			      payload_with_crc_len, whitener_offset, showWhitenUnmake);
    showWhitenUnmake = false;
  } else {
    payload_with_crc.reserve(payload_with_crc_len);
    payload_with_crc.append(whitened_payload_with_crc, payload_with_crc_len);
  }

  payload.reserve(payload.length() + payload_len);
  payload.append(payload_with_crc, 0, payload_len);

  // read network order crc
  uint32_t expected_crc = 0;
  for (size_t i = 0; i < 4; i++) {
    unsigned char c = payload_with_crc[payload_len + i];
    expected_crc |= ((c & 0xFF) << (8 * i));
  }
 
  // convert expected crc to host order
  expected_crc = ntohl(expected_crc);

  uint32_t actual_crc = digital_crc32(payload);
  ok = (actual_crc == expected_crc);

  if ((0 && VERBOSE) || toast3) {
    fprintf(stderr, "unmake_packet: expected=%u, expected=%.8X, actual=%u, actual=%.8X, len=%d, ok=%d\n", expected_crc, expected_crc, actual_crc, actual_crc, payload.length(), ok);
...
std::string whiten(const char* s, size_t s_len, int o, bool showWhiten) {
  std::string ws;

  if (showWhiten) {
    fprintf(stderr, "pre-whiten: ");
    for (int i = 0; i < 30; i++)
  ...
  for (int i = 0; i < s_len; i++)
    ws.push_back(s[i] ^ random_mask_tuple[i + o]);

  if (showWhiten) {
    fprintf(stderr, "pos-whiten: ");
    for (int i = 0; i < 30; i++)
      fprintf(stderr, "%02X", (unsigned char) ws[i]);
    fprintf(stderr, "\n");
  }
...
const unsigned char random_mask_tuple[] = {
